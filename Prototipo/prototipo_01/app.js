(() => {
  ...
  function analyze(period){
    ...
    const expenseTotal=expenses.reduce((s,m)=>s+m.value,0);
    const diff=incomeTotal-expenseTotal;
    const ratio=incomeTotal>0 ? (expenseTotal/incomeTotal)*100 : null;
    const byCategory={};
    expenses.forEach(m=>byCategory[m.category]=(byCategory[m.category]||0)+m.value);
    return {incomes,expenses,incomeTotal,expenseTotal,diff,ratio,byCategory};
  }

  // ---------- Dashboard ----------
  function renderDashboard(){
    const a=analyze(state.selectedPeriod);
    $("#period-button").textContent=periodLabel(state.selectedPeriod);
    ...
    $("#expense-total").textContent=money(a.expenseTotal);

    if(a.incomeTotal>0){
      if(a.diff>=0){
        $("#difference-label").textContent="Sobra da renda";
        $("#difference-total").textContent=money(a.diff);
      } else {
        $("#difference-label").textContent="Despesas acima da renda";
        $("#difference-total").textContent=money(Math.abs(a.diff));
      }
      $("#ratio-message").innerHTML=`Suas despesas representam <strong>${Math.round(a.ratio)}%</strong> da sua renda neste mês.`;
    } else {
      $("#difference-label").textContent="Renda não informada";
      $("#difference-total").textContent="—";
      $("#ratio-message").textContent="Nenhuma renda informada neste mês.";
    }

    const entries=Object.entries(a.byCategory).sort((x,y)=>y[1]-x[1]);
    $("#category-list").innerHTML=entries.length
      ? entries.map(([cat,total])=>`<button class="category-row" data-category="${cat}"><span><strong>${cat}</strong><br><small>${a.expenses.filter(e=>e.category===cat).length} movimentação(ões)</small></span><strong>${money(total)}</strong></button>`).join("")
      : `<p class="muted">Nenhuma despesa registrada neste mês.</p>`;
  ...
  $("#prev-month").addEventListener("click",()=>{state.selectedPeriod=shiftPeriod(state.selectedPeriod,-1);renderDashboard();});
  $("#next-month").addEventListener("click",()=>{state.selectedPeriod=shiftPeriod(state.selectedPeriod,1);renderDashboard();});
  $("#period-button").addEventListener("click",()=>{state.pickerYear=Number(state.selectedPeriod.slice(0,4));renderMonthPicker();showScreen("screen-month-picker");});
  $("#income-card").addEventListener("click",()=>openList("income",null));
  $("#expense-card").addEventListener("click",()=>openList("expense",null));

  // ---------- Month picker ----------
  function renderMonthPicker(){
    $("#picker-year").textContent=state.pickerYear;
    $("#month-grid").innerHTML=shortMonthNames.map((m,i)=>{
      const p=`${state.pickerYear}-${String(i+1).padStart(2,"0")}`;
      return `<button class="month-btn ${p===state.selectedPeriod?"selected":""}" data-period="${p}">${m}</button>`;
    }).join("");
    $$("#month-grid [data-period]").forEach(b=>b.addEventListener("click",()=>{state.selectedPeriod=b.dataset.period;openDashboard();}));
  }
  $("#picker-prev-year").addEventListener("click",()=>{state.pickerYear--;renderMonthPicker();});
  $("#picker-next-year").addEventListener("click",()=>{state.pickerYear++;renderMonthPicker();});

  // ---------- Add sheet ----------
  $("#open-add-sheet").addEventListener("click",()=>setOverlay("sheet-backdrop",true));
  ...
  $$(".back-context").forEach(b=>b.addEventListener("click",()=>showScreen(state.returnScreen)));
  ...
  $("#expense-form").addEventListener("submit",(e)=>{
    ...
    if(state.editing){
      ...
      if(m.recurring){
        ...
        openList("expense",state.listContext.category);
        return;
      }
    }
  ...
  // ---------- Lists ----------
  function openList(type,category=null){
    state.listContext={type,category};
    const items=movementsForPeriod(state.selectedPeriod,type).filter(m=>!category || m.category===category);
    $("#list-eyebrow").textContent=periodLabel(state.selectedPeriod);
    $("#list-title").textContent=category || (type==="income" ? "Rendas":"Despesas");
    $("#movement-list").innerHTML=items.length
      ? items.map(m=>`
        <button class="movement-row" data-id="${m.id}">
          <span>
  ...
  $("#detail-back").addEventListener("click",()=>openList(state.listContext.type,state.listContext.category));
  ...
  $("#scope-this-month").addEventListener("click",()=>{
    ...
    openList(state.listContext.type,state.listContext.category);
  });
  ...
  $("#explore-first").addEventListener("click",()=>{loadDemoIfEmpty();openDashboard();});
